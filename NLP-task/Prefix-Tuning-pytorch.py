...
# 设置随机数种子
setup_seed(20)
def evaluate(train_data,test_data_batch_A_dev,test_data_batch_maskedX_dev, train_label, model, device):
    # 设置模型为评估模式
    model.eval()
    ...
    with torch.no_grad():
        # 假设模型返回预测结果和损失值
        # print('test_data_batch_maskedX_dev.shape',test_data_batch_maskedX_dev.shape)
        out,_ = model(train_data,test_data_batch_A_dev,test_data_batch_maskedX_dev)
        ...
        acc = metrics.accuracy_score(preds, train_label)
        auc = 0
        try:
            auc = metrics.roc_auc_score(preds, train_label, average='macro', sample_weight=None)  ## y_true=ground_truth
        except ValueError:
            pass
        test_result = [kfold_index, metrics.precision_score(preds, train_label), metrics.recall_score(preds, train_label),
                       metrics.accuracy_score(preds, train_label),
                       metrics.f1_score(preds, train_label), auc]

    # 返回评估结果（损失和其他指标）
    return acc, test_result

# BD load  导入BD_sig.npy
print('loading BD data...')
...
epochs_rec = 30  # 20 678 # 10 684 # 0 663 # 30 666 # 15 678 5 666
epochs = 90 + epochs_rec  # 116 671
    ...
    for trainval_index, test_index in kf.split(X, Y):
        ...
        for k in range(X_avg.shape[0]):
            for i in range(X_avg.shape[1]):
                if abs(X_avg[k][i] < 0.15):  # 0.2 678  0.4 646 0.3 678 0.1 678 0.15 696 ！！！！！
                    X_avg[k][i] = 0
                else:
                    X_avg[k][i] = 1
        ...
        #state_dict = torch.load('./checkpoint1//1.pt')
        state_dict = torch.load('../models///1.pt')
        model.load_state_dict(state_dict)
        model.to(device)

        # Separate the model parameters into bias and non-bias parameters
        bias_params = []
        non_bias_params = []
        ...
        #print(total)343758
        #bias.named_parameters()
        total1 = sum([param.nelement() for param in bias_params])
        #print(total1)282
        #non_bias_params.named_parameters()
        total2 = sum([param.nelement() for param in non_bias_params])
        #print(total2)343476
        ...
        optimizer2 = optim.SGD(bias_params, lr=lr, weight_decay=decay, momentum=0.9, nesterov=True)
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % 10 == 0:
                ...
                train_loss, train_result = evaluate(train_data_batch_dev, train_data_batch_A_dev, train_data_batch_maskedX_dev
                                                    ,train_label_batch, model, device)
                print('train:', train_result)

            if epoch % 120 == 0 and epoch >=epochs_rec:
                # model.eval()
                ...
                if True:  # acc_val > best_val:
                    ...
                    auc = 0
                    try:
                        auc = metrics.roc_auc_score(preds, Y_test, average='macro',
                                                    sample_weight=None)  ## y_true=ground_truth
                    except ValueError:
                        pass
                    ...
                    #                }
                    # print(test_result)
                    test_result = [kfold_index, metrics.precision_score(preds, Y_test),
                                   metrics.recall_score(preds, Y_test), metrics.accuracy_score(preds, Y_test),
...
f.save('BrainGSL_pytorch_1_result.xls')#保存.xls到当前工作目录
